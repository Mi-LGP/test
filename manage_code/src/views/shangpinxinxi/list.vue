<template>
  <div>
    <div class="center_view">
      ...
      <div class="dataList" v-if="btnAuth('shangpinxinxi', '查看')">
        <div class="item" v-for="(item, index) in list" :key="item.id">
          <template v-if="item.shangpintupian">
            <el-image
              v-if="item.shangpintupian.substring(0, 4) == 'http'"
              preview-teleported
              :preview-src-list="[item.shangpintupian.split(',')[0]]"
              :src="item.shangpintupian.split(',')[0]"
            ></el-image>
            <el-image
              v-else
              preview-teleported
          ...
          <div class="btns">
            ...
            <el-button
              class="cross_btn"
              v-if="btnAuth('shangpinxinxi', '发布团购')"
              type="success"
              @click="tuangoushangpinCrossAddOrUpdateHandler(item, 'cross', '', '', '', '')"
            >
              发布团购
            </el-button>
            <el-button
              class="cross_btn"
              v-if="btnAuth('shangpinxinxi', '配送')"
              type="success"
              @click="dingdanpeisongCrossAddOrUpdateHandler(item, 'cross', '', '', '', '')"
            >
              配送
            </el-button>
            <el-button
              class="operate_btn"
              v-if="btnAuth('shangpinxinxi', '查看评论')"
              type="warning"
    ...
    <el-dialog v-model="chatVisible" @close="clearChat" :title="nowname" :append-to-body="true">
      <div class="chat-content" id="chat-content">
        <div v-bind:key="item.id" v-for="(item, index) in chatList">
          ...
          <div v-if="item.uid == user.id" class="right-content">
            ...
            ></el-image>
            <img
              :src="avatar ? $config.url + avatar : require('@/assets/img/avatar.png')"
              alt=""
              style="
                width: 30px;
                border-radius: 50%;
          ...
          <div v-else class="left-content">
            <img
              :src="nowfpic ? $config.url + nowfpic : require('@/assets/img/avatar.png')"
              alt=""
              style="
                width: 30px;
                border-radius: 50%;
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button :disabled="chatForm.content ? false : true" type="primary" @click="addChat"
          >发送</el-button
        >
        <el-upload
    ...
    <tuangoushangpinFormModel
      ref="tuangoushangpinFormModelRef"
      @formModelChange="formModelChange"
    ></tuangoushangpinFormModel>
    <dingdanpeisongFormModel
      ref="dingdanpeisongFormModelRef"
      @formModelChange="formModelChange"
    ></dingdanpeisongFormModel>
  </div>
</template>
...
import { reactive, ref, getCurrentInstance, nextTick, onMounted, watch, computed } from 'vue'
...
const tuangoushangpinCrossAddOrUpdateHandler = (
  row,
  type,
  crossOptAudit,
  crossOptPay,
  ...
  nextTick(() => {
    tuangoushangpinFormModelRef.value.init(
      row.id,
      'cross',
      '发布团购',
      row,
...
const dingdanpeisongCrossAddOrUpdateHandler = (
  row,
  type,
  crossOptAudit,
  crossOptPay,
  ...
  nextTick(() => {
    dingdanpeisongFormModelRef.value.init(
      row.id,
      'cross',
      '配送',
      row,
