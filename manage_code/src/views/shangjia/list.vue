      ...
      <div class="list_search_view">
        <el-form :model="searchQuery" class="search_form">
          <div class="search_view">
            <div class="search_label">商家：</div>
            <div class="search_box">
              <el-input
                class="search_inp"
                v-model="searchQuery.shangjia"
                placeholder="商家"
                clearable
              >
              </el-input>
      ...
      >
        ...
        >
          <template #default="scope">
            ...
            <el-button
              class="operate_btn"
              v-if="btnAuth('shangjia', '私信')"
              type="success"
              @click="chatClick(scope.row)"
            >
              私信
            </el-button>
          </template>
...
const getList = () => {
  ...
  params['sort'] = 'id'
  params['order'] = 'desc'
  if (searchQuery.value.shangjia && searchQuery.value.shangjia != '') {
    params['shangjia'] = '%' + searchQuery.value.shangjia + '%'
  }
  context
    .$http({
      url: `${tableName}/page`,
...
const exportClick = () => {
  import('@/utils/Export2Excel').then(excel => {
    const tHeader = ['商家', '密码', '商家名称', '性别', '头像', '年龄']
    const filterVal = ['shangjia', 'mima', 'shangjiamingcheng', 'xingbie', 'touxiang', 'nianling']
