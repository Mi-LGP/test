<template>
  <div>
    <div class="center_view">
      <el-tabs v-model="orderStatus" type="card" class="demo-tabs" @tab-change="orderTabClick">
        <el-tab-pane label="全部" :name="''"></el-tab-pane>
        <el-tab-pane
          label="未支付"
          name="未支付"
          v-if="btnAuth(`orders/未支付`, '查看')"
        ></el-tab-pane>
        <el-tab-pane
          label="拼团中"
          name="拼团中"
          v-if="btnAuth(`orders/拼团中`, '查看')"
        ></el-tab-pane>
        <el-tab-pane
          label="已支付"
          name="已支付"
          v-if="btnAuth(`orders/已支付`, '查看')"
        ></el-tab-pane>
        <el-tab-pane
          label="已发货"
          name="已发货"
          v-if="btnAuth(`orders/已发货`, '查看')"
        ></el-tab-pane>
        <el-tab-pane
          label="已完成"
          name="已完成"
          v-if="btnAuth(`orders/已完成`, '查看')"
        ></el-tab-pane>
        <el-tab-pane
          label="已退款"
          name="已退款"
          v-if="btnAuth(`orders/已退款`, '查看')"
        ></el-tab-pane>
        <el-tab-pane
          label="已取消"
          name="已取消"
          v-if="btnAuth(`orders/已取消`, '查看')"
        ></el-tab-pane>
      </el-tabs>
      <div class="list_search_view">
        <el-form :model="searchQuery" class="search_form">
          <div class="search_view">
            <div class="search_label">订单编号：</div>
        ...
        <div class="btn_view">
          ...
          <el-button
            ...
            @click="delClick(null)"
            v-if="btnAuth('orders' + (orderStatus ? '/' + orderStatus : ''), '删除')"
          >
          ...
          <el-button
            ...
            :disabled="selRows.length ? false : true"
            @click="exportClick"
            v-if="btnAuth('orders' + (orderStatus ? '/' + orderStatus : ''), '导出')"
          >
          ...
          <el-button
            ...
            type="warning"
            @click="echartClick1"
            v-if="btnAuth('orders' + (orderStatus ? '/' + orderStatus : ''), '月销量')"
          >
          ...
          <el-button
            ...
            type="warning"
            @click="echartClick2"
            v-if="btnAuth('orders' + (orderStatus ? '/' + orderStatus : ''), '日销量')"
          >
            <i class="iconfont icon-bar-chart--line"></i>
            日销量
          </el-button>
      ...
      >
        ...
        <template v-if="!orderStatus || orderStatus == '已完成' || orderStatus == '已支付'">
          ...
          <el-table-column
            label="审核"
            v-if="btnAuth('orders' + (orderStatus ? '/' + orderStatus : ''), '审核')"
            :resizable="true"
            :sortable="true"
            align="left"
            header-align="left"
        ...
        >
          <template #default="scope">
            ...
            <el-button
              ...
              v-if="btnAuth('orders' + (orderStatus ? '/' + orderStatus : ''), '修改')"
            >
              修改
            </el-button>
            <el-button
            ...
            <el-button
              class="operate_btn"
              v-if="btnAuth('orders' + (scope.row.status ? '/' + scope.row.status : ''), '物流')"
              type="warning"
            ...
            <el-button
              class="operate_btn"
              v-if="btnAuth('orders' + (scope.row.status ? '/' + scope.row.status : ''), '发货')"
              type="warning"
              @click="sendGoodClick(scope.row)"
            >
              发货
            </el-button>
            <el-button
              class="operate_btn"
              v-if="
                btnAuth('orders' + (scope.row.status ? '/' + scope.row.status : ''), '确认收货') &&
                scope.row.userid == user.id
              "
              type="warning"
              @click="confirmGoodClick(scope.row)"
            >
              确认收货
            </el-button>
          </template>
...
if (
  ['未支付', '已支付', '已发货', '已完成', '已退款', '已取消', '拼团中'].includes(
    route.query.menuJump
  )
) {
