    ...
    >
      <el-form class="formModel_form" ref="formRef" :model="form" :rules="rules">
        <el-row>
          ...
          <el-col :span="24">
            <el-form-item label="支付类型" prop="type">
              <el-select
                ...
                placeholder="请选择支付类型"
              >
                <el-option v-for="(item, index) in typeLists" :label="item" :value="index + 1 + ''">
                </el-option>
              </el-select>
            </el-form-item>
          </el-col>
          ...
          <el-col :span="24">
            <el-form-item label="商户名称" prop="shangjia">
              <el-input
                class="list_inp"
          ...
          <el-col :span="24">
            <el-form-item label="团购编号" prop="groupno">
              <el-input
                ...
                placeholder="团购编号"
                type="text"
                :readonly="!isAdd || disabledForm.groupno ? true : false"
              />
            </el-form-item>
          </el-col>
          ...
          <el-col :span="24">
            <el-form-item label="下单时间" prop="addtime">
              <el-input class="list_inp" v-model="form.addtime" placeholder="下单时间" readonly />
            </el-form-item>
          </el-col>
          <el-col :span="24">
...
const rules = ref({
  ...
  buynumber: [{ validator: context.$toolUtil.validator.intNumber, trigger: 'blur' }],
  price: [{ validator: context.$toolUtil.validator.number, trigger: 'blur' }],
  discountprice: [{ validator: context.$toolUtil.validator.number, trigger: 'blur' }],
  total: [{ validator: context.$toolUtil.validator.number, trigger: 'blur' }],
  discounttotal: [{ validator: context.$toolUtil.validator.number, trigger: 'blur' }],
  type: [],
  status: [],
  address: [],
  tel: [],
  ...
  context
    ...
    .then(res => {
      ...
      res.data.data.logistics = res.data.data.logistics
        ? res.data.data.logistics.replace(reg, '../../../cl636300194/file')
        : ''
...
const init = (
  ...
  if (formType == 'add') {
    ...
    for (let x in row) {
      ...
      if (x == 'groupno') {
        form.value.groupno = row[x]
        disabledForm.value.groupno = true
        continue
      }
    }
    if (row) {
