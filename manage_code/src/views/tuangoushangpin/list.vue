	...
	<div>
		<div class="center_view">
			<div class="list_search_view">
				<el-form :model="searchQuery" class="search_form" >
					<div class="search_view">
						...
						<div class="search_box">
							<el-input class="search_inp" v-model="searchQuery.shangpinbianhao" placeholder="商品编号"
								clearable>
							</el-input>
						</div>
					</div>
				...
				<div class="btn_view">
					<el-button class="add_btn" type="success" @click="addClick" v-if="btnAuth('tuangoushangpin','新增')">
			...
			<div class="dataList"
				 v-if="btnAuth('tuangoushangpin','查看')"
			>
				<div class="item" v-for="(item,index) in list" :key="item.id">
					...
					<div class="btns">
						<el-button class="view_btn" type="info" v-if=" btnAuth('tuangoushangpin','查看')" @click="infoClick(item.id)">
							详情
						</el-button>
						<el-button class="edit_btn" type="primary" @click="editClick(item.id)" v-if=" btnAuth('tuangoushangpin','修改')">
							修改
						</el-button>
						<el-button class="del_btn" type="danger" @click="delClick(item.id)"  v-if="btnAuth('tuangoushangpin','删除')">
							删除
						</el-button>
						<el-button class="cross_btn" v-if="btnAuth('tuangoushangpin','配送')" type="success" @click="dingdanpeisongCrossAddOrUpdateHandler(item,'cross','','','','')">
							配送
						</el-button>
						<el-button class="operate_btn" v-if="btnAuth('tuangoushangpin','查看评论')" type="warning" @click="commentClick(item.id)">
							查看评论
						</el-button>
                        <el-button class="operate_btn" v-if="btnAuth('tuangoushangpin','私信')" type="success" @click="chatClick(item)">
                          私信
                        </el-button>
					</div>
				</div>
		...
		<dingdanpeisongFormModel ref="dingdanpeisongFormModelRef" @formModelChange="formModelChange"></dingdanpeisongFormModel>
	</div>
</template>
<script setup>
	...
	const commentClick=(id)=>{
		context?.$router.push('/discusstuangoushangpin?refid=' + id)
	}
	import dingdanpeisongFormModel from '@/views/dingdanpeisong/formModel'
	const dingdanpeisongFormModelRef = ref(null)
    const dingdanpeisongCrossAddOrUpdateHandler = (row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue) => {
		...
		nextTick(()=>{
			dingdanpeisongFormModelRef.value.init(row.id,'cross','配送',row,'tuangoushangpin',statusColumnName,tips,statusColumnValue)
		})
    }
    ...
    const chatClick = (row) => {
        if (row.shangjia == user.value.shangjia) {
        ...
        context.$http.get('shangjia/query', {
			...
			if (res.data && res.data.code == 0) {
                ...
                nowname.value = res.data.data.shangjia
	...
	//初始化
	const init = () => {
		let urlalllimittimes = 'tuangoushangpin/remind/alllimittimes/1'
		urlalllimittimes+=`?remindend=50`
