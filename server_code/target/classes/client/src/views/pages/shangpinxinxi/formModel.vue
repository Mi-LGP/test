	...
	<div class="detail-page" :style='{}'>
		...
		<div class="detail_view">
			...
			<div class="info_view">
				...
				<div class="btn_view">
					<el-button class="edit_btn" type="primary" v-if="btnAuth('shangpinxinxi','私信')" @click="chatClick">联系TA</el-button>
					<el-button class="cross_btn" v-if="btnFrontAuth('shangpinxinxi','发布团购')" @click="tuangoushangpinonAcross('发布团购','','','','')" type="warning">发布团购</el-button>
					<el-button class="cross_btn" v-if="btnFrontAuth('shangpinxinxi','配送')" @click="dingdanpeisongonAcross('配送','','','','')" type="warning">配送</el-button>
...
<script setup>
	...
	const tuangoushangpinonAcross = async (btnType,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1) => {
		if(!context?.$toolUtil.storageGet('frontToken')){
			context?.$toolUtil.message('请登录后再操作！','error')
			return false
		}
		if(!btnAuth('shangpinxinxi',btnType)){
		...
		nextTick(()=>{
			router.push(`/index/tuangoushangpinAdd?type=cross&&id=${detail.value.id}`)
		})
	}
