<template>
  <div>
    <div class="panel">
      ...
      >
        <el-table-column prop="sjd" label="时间段" width="80" align="center"> </el-table-column>
...
export default {
  ...
  methods: {
    ...
    tableCellStyle(row) {
      if (
        row.column.property != 'jc' &&
        row.column.property != 'sjd' &&
        row.row[row.column.property].title !== undefined
      ) {
        return {
    ...
    makeTimetable() {
      ...
      for (let i = 0; i < this.time.length; i++) {
        let one = {
          sjd: this.time[i].sjd,
          jc:
            Number(this.time[i].sectionnum) +
            '<br><div style="font-size: 12px">(' +
            this.time[i].starttime +
            '-' +
            this.time[i].endtime +
            ')</div>',
          jc1: i + 1,
    ...
    objectSpanMethod({ row, column, rowIndex, columnIndex }) {
      ...
      if (columnIndex === 0) {
        if (rowIndex < this.morningLength) {
          ...
          rowIndex > this.morningLength - 1 &&
          rowIndex < this.morningLength + this.afternoonLength
        ) {
          if (rowIndex === this.morningLength) {
            return {
