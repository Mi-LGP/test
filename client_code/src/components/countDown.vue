<template>
  <div>
    <p v-if="msTime.show" style="margin: 0">
      ...
      ><i>{{ hourTxt }}</i> <span>{{ msTime.minutes }}</span
      ><i>{{ minutesTxt }}</i> <span>{{ msTime.seconds }}</span
      ><i>{{ secondsTxt }}</i>
    </p>
...
const gogogo = () => {
  //判断是秒还是毫秒
  startTime.value.toString().length == 10
    ? (star.value = startTime.value * 1000)
    : (star.value = startTime.value)
  endTime.value.toString().length == 10
    ? (end.value = endTime.value * 1000)
    : (end.value = endTime.value)
  ...
  if (end.value < current.value) {
    ...
    (end.value > current.value && star.value < current.value) ||
    star.value == current.value
  ) {
    /**
