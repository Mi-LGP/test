  ...
  >
    ...
    >
      ...
      <el-tab-pane
        ...
        name="未支付"
      ></el-tab-pane>
      <el-tab-pane label="拼团中" name="拼团中"></el-tab-pane>
    ...
    >
      ...
      <el-table-column label="价格" :resizable="true" align="left" header-align="left">
        <template #default="scope">
          <span v-if="scope.row.type != 2" style="font-size: 12px">￥</span>{{ scope.row.price }}
          <span v-if="scope.row.type == 2">积分</span>
        </template>
      </el-table-column>
      <el-table-column label="总价" :resizable="true" align="left" header-align="left">
        <template #default="scope">
          <span v-if="scope.row.type != 2" style="font-size: 12px">￥</span>{{ scope.row.total }}
          <span v-if="scope.row.type == 2">积分</span>
        </template>
      </el-table-column>
      ...
      >
        <template #default="scope">
          ...
          <el-button
            ...
            type="danger"
            @click="refundPriceClick(scope.row)"
          >
            退款
          </el-button>
          <template v-if="!scope.row.sfsh || scope.row.sfsh == '否'">
            <el-button
              ...
              v-if="scope.row.status == '已完成' && scope.row.sfsh != '否'"
              type="danger"
              @click="refundGoodClick(scope.row)"
            >
              退货
            </el-button>
            <el-button
              class="refundGood_btn"
              v-else-if="scope.row.sfsh == '否'"
              type="danger"
              @click="refundGoodClick(scope.row)"
            >
          ...
          <el-button
            class="cancel_btn"
            v-if="scope.row.status == '拼团中'"
            type="danger"
            @click="cancelGroupClick(scope.row)"
          >
            取消拼团
          </el-button>
          <el-button
...
if (
  ['未支付', '拼团中', '已支付', '已发货', '已完成', '已退款', '已取消'].includes(
    ...
    .then(() => {
      context
        ...
        .then(res => {
          ...
          //减去用户余额
          userinfo.value.money = (parseFloat(userinfo.value.money) - parseFloat(row.total)).toFixed(
            2
          )
          //如果商品存在库存，则减去商品库存
...
// 取消拼团
const cancelGroupClick = row => {
  ElMessageBox.confirm(`是否取消该拼团订单？`, '提示', {
    confirmButtonText: '是',
    cancelButtonText: '否',
    type: 'warning',
  })
          ...
          context
            ...
            .then(obj => {
              ...
              context
                ...
                .then(res1 => {
                  context.$message.success('取消拼团成功')
                  getSession()
                  statusChange()
                })
            })
        ...
        td {
          ...
          .cell {
            ...
            .refundPrice_btn:hover {
              background: rgba(240, 160, 140, 0.8);
            }
            // 退货
            .refundGood_btn {
              border: 0;
            ...
            // 物流-悬浮
            .logistics_btn:hover {
              background: rgba(80, 180, 220, 0.8);
            }
            // 确认收货
            .confirm_btn {
              border: 0;
            ...
            // 确认收货-悬浮
            .confirm_btn:hover {
              background: rgba(65, 128, 230, 0.8);
            }
            // 评论
            .toDetail_btn {
              border: 0;
