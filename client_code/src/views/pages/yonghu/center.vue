  ...
  <div class="center_view" :style="{}">
    ...
    <div class="usersView">
      <div class="usersTabView">
        ...
        <div
          class="usersTab"
          :class="tabIndex == 'chat' ? 'usersTabActive' : ''"
          @click="tabClick({ tableName: 'chat' })"
        >
          聊天记录
        </div>
      ...
      <div class="usersBox updateInfo" v-if="tabIndex == 'center'">
        ...
        >
          <el-row>
            <el-col :span="24">
              <el-form-item prop="yonghu" label="用户">
                <el-input
                  class="list_inp"
                  v-model="userForm.yonghu"
                  placeholder="用户"
                  readonly
                ></el-input>
              </el-form-item>
      ...
      <div class="usersBox chatRecord" v-if="tabIndex == 'chat'">
        <div class="z-box" :style="{ width: '70%', padding: '20px', margin: '0 auto' }">
          ...
          >
            <img
              :src="item.picture ? $config.url + item.picture : require('@/assets/avatar.png')"
            ...
            >
              <div
                :style="{ fontSize: '14px', color: '#000', flex: '1', fontWeight: 'bold' }"
                class="item-style"
              >
                {{ item.name }}
              </div>
              ...
              >
                ...
                {{
                  item.content.split('/').length > 1 && item.content.split('/')[0] == 'upload'
                    ? '[图片]'
                    : item.content
                }}
              </div>
          ...
          <div class="yinhang_view" v-show="payType == 3">
            ...
            <div class="centerPayView">
              <el-radio v-model="payType1" label="3"
                ><img src="@/assets/pay/jianshe.png" alt />中国建设银行</el-radio
              >
            </div>
            <div class="centerPayView">
              <el-radio v-model="payType1" label="4"
                ><img src="@/assets/pay/gonghang.png" alt />中国工商银行</el-radio
              >
            </div>
            <div class="centerPayView">
    ...
    <el-dialog v-model="chatVisible" @close="clearChat" :title="nowname">
      <div class="chat-content" id="chat-content">
        <div v-bind:key="item.id" v-for="(item, index) in chatList">
          ...
          <div v-if="item.uid == userForm.id" class="right-content">
            ...
            ></el-image>
            <img
              :src="mypic ? $config.url + mypic : require('@/assets/avatar.png')"
              alt=""
              style="
                width: 30px;
                border-radius: 50%;
          ...
          <div v-else class="left-content">
            <img
              :src="nowfpic ? $config.url + nowfpic : require('@/assets/avatar.png')"
              alt=""
              style="
                width: 30px;
                border-radius: 50%;
      ...
      <div slot="footer" class="dialog-footer">
        <el-input
          ...
          placeholder="请输入内容"
          style="width: calc(100% - 180px); float: left; border: 1px solid #ccc; margin-right: 10px"
        >
        </el-input>
...
//验证规则
const rules = ref({
  yonghu: [{ required: true, message: '请输入', trigger: 'blur' }],
  mima: [{ required: true, message: '请输入', trigger: 'blur' }],
  xingming: [{ required: true, message: '请输入', trigger: 'blur' }],
  xingbie: [],
  touxiang: [],
  nianling: [{ validator: context.$toolUtil.validator.intNumber, trigger: 'blur' }],
  money: [{ validator: context.$toolUtil.validator.number, trigger: 'blur' }],
})
const getSession = () => {
  context
    ?.$http({
...
const tabClick = item => {
  ...
  router.push(
    `/index/${item.classname || item.tableName}List?centerType=1${item.menuJump ? '&menuJump=' + item.menuJump : ''}`
  )
}
const hasBack = menu => {
