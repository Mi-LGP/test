		...
		<div class="detail_view">
			...
			<div class="info_view">
				...
				<div class="btn_view">
					...
					<el-button class="openGroup_btn" v-if="detail.grouppeople>1" type="primary" @click="groupBuyTap">我要开团</el-button>
					<el-button class="goGroup_btn" v-if="detail.grouppeople>1" type="success" @click="showGroupListDialog">去参团</el-button>
				</div>
				<div class="btn_view">
					<el-button class="edit_btn" type="primary" v-if="btnAuth('tuangoushangpin','私信')" @click="chatClick">联系TA</el-button>
					<el-button class="cross_btn" v-if="btnFrontAuth('tuangoushangpin','配送')" @click="dingdanpeisongonAcross('配送','','','','')" type="warning">配送</el-button>
					<el-button class="edit_btn" v-if="centerType&&btnAuth('tuangoushangpin','修改')" type="primary" @click="editClick">修改</el-button>
					<el-button class="del_btn" v-if="centerType&&btnAuth('tuangoushangpin','删除')" type="danger" @click="delClick">删除</el-button>
				</div>
			</div>
		</div>
				...
				<div class="group_list_view">
					<el-table :data="groupList" stripe style="width: 100%">
						...
						<el-table-column prop="status" label="状态" width="100">
							<template #default="scope">
								<el-tag :type="scope.row.status === '拼团中' ? 'success' : 'info'">{{scope.row.status}}</el-tag>
							</template>
						</el-table-column>
						...
						<el-table-column label="操作" fixed="right">
							<template #default="scope">
								<el-button
									...
									v-if="scope.row.status === '拼团中' && scope.row.curpeople < scope.row.grouppeople"
								>
									立即参团
								</el-button>
								<el-button
...
<script setup>
	...
	const dingdanpeisongonAcross = async (btnType,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1) => {
		if(!context?.$toolUtil.storageGet('frontToken')){
